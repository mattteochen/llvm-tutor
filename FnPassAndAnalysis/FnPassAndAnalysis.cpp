...
//-----------------------------------------------------------------------------
// FnPassAndAnalysis implementation
//-----------------------------------------------------------------------------
...
struct FnPassAndAnalysis : PassInfoMixin<FnPassAndAnalysis> {
...
llvm::PassPluginLibraryInfo getHelloWorldPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "FnPassAndAnalysis", LLVM_VERSION_STRING,
          [](PassBuilder &PB) {
            PB.registerPipelineParsingCallback(
                [](StringRef Name, FunctionPassManager &FPM,
                   ArrayRef<PassBuilder::PipelineElement>) {
                  if (Name == "fn-pass-and-analysis") {
                    FPM.addPass(FnPassAndAnalysis());
                    return true;
                  }
                  return false;
                });
...
// be able to recognize FnPassAndAnalysis when added to the pass pipeline on the
